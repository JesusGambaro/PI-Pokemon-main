...
import {addPokemon} from "../Redux/actions/addPokemon";
...
const CreateNew = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setErrors({
            ...errors, name: validation(data.name, "name"),
        ...
        if (Object.values(errors).some((e) => e.length) ||
            Object.values(data).slice(0, 8).some((d) => d === "" || !d.length)) return;
        ...
        switch (type) {
            case "sprites":
                return !/(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/
                    .test(param) ? "Insert a valid URL" : "";
            case "name":
                return !/^[a-zA-Zs]*$/.test(param) ? "Must be just characters"
                    : param.length < 3 ? "Minimum length 3" :
                        param.length > 10 ? "Maximum length 10" :
                            "";
            default:
                if (!/^[0-9]+$/.test(param)) {
    ...
    const handleInputChange = (e) => {
        ...
        setErrors({...errors, [e.target.name]: validation(e.target.value, e.target.name)});
    }
    //-------------TYPES-------------//
    ...
    useEffect(() => {
        ...
        else setErrors({...errors, types: data.types.length < 1 ? "Almost 1 needed" : ""});
    }, [data.types])
                    ...
                    <div className="types">
                        ...
                        <div className="selected-types">
                            {data.types.map((type, i) => {
                                return (
                                    <span key={i} className="type" style={{"--i": typesColors[type]}}>
                                        {type.charAt(0).toUpperCase() + type.substring(1)}
                                        <button className="delete-type"
                                                type="button"
                                                onClick={() => handleDeleteType(type)}>X</button>
                                    </span>
                                );
                            })}
